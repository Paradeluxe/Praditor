...
`LowPass≤f_Nyquist=(Sample Rate)/2`  

If the user ultimately chooses a LowPass frequency that exceeds Nyquist frequency, Praditor will automatically apply the bandpass filter using the highest valid frequency instead.  

Most of the time, tuning these two parameters will not have any significant impact on the annotation. However, they become particularly useful when recording in an environment with consistent noise interference in a certain frequency band, such as the hum from an air conditioner.  


![example_LowHighPass.png](../instructions/example_LowHighPass.png)
 
*An example of tuning LowPass. HighPass works the similar way.*
...
*Suggested RefLen (blue) and extreme values to be avoided (red) in the original signal form.*
...
*An example of tuning Threshold. The first annotation is blocked by an abrupt noise spike due to Threshold being too low.*
...
*An example of tuning KernelSize.*


![example_KernelFrm.png](../instructions/example_KernelFrm.png)

*An example of tuning KernelFrm%.*
...
*An example of tuning CountValid. The first spike is likely to be lip-smacking.*

...
def process_item(params_procitem, audio_file, reverse):
    ...
    if reverse:
        target_audio_arr_filtered_ = np.flip(target_audio_arr_filtered_)
    ...
    target_audio_arr_ds = np.max(target_audio_arr_ds, axis=1)  # 用max方法降采样
    ...
    # 0.015
    eps = eps_radius_ratio * float(np.max(np.sort(target_audio_arr_ds)[:int(.8 * len(target_audio_arr_ds))]))  # 防止异常值

    min_samples = math.ceil(0.3/accFactor*target_audio_samplerate) #math.ceil(2 / (target_audio_samplerate/44100) / (interval*2/4281))

    cluster = DBSCAN(eps=eps, min_samples=min_samples, metric="manhattan").fit(points_array)
    ...
    for offset, onset in onoffsets:
        ...
        sample_startpoint = int(np.argmin(candidate_y1_area) + offset * accFactor)
        ...
        candidate_y1_area = np.sort(candidate_y1_area)[:int(len(candidate_y1_area) * ratio)]

        y1_threshold = float(np.sum(candidate_y1_area)/(sample_startpoint-sample_endpoint) * amp)
